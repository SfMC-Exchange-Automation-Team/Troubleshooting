...
| `Evidence\UpdateInventory.json` | JSON | Full Exchange CU/HU/SU inventory object. |
| `Evidence\UpdateInventory.csv` | CSV | Flattened Exchange update inventory rows. |
| `Evidence\Preflight.json` | JSON | Full preflight object, including pending reboot state, .NET readiness, and .NET acceleration placeholder state. |
| `Evidence\Preflight.csv` | CSV | Flattened pending reboot and .NET readiness rows. |
| `Summary.csv` | CSV | Manager-friendly summary of Stage 1 findings. |
| `GuiConfig\ExchangeCuPatch.gui.<CorrelationId>.psd1` | PowerShell data file | Generated by the GUI wizard when an operator runs the toolbox from the GUI. |
